...
static void draw_battery(lv_obj_t* canvas, uint8_t level, bool usb_present) {
    if (level < CONFIG_DONGLE_SCREEN_BATTERY_CRIT_LEVEL) {
        lv_canvas_fill_bg(canvas, lv_palette_main(LV_PALETTE_RED), LV_OPA_COVER);
    } else if (level <= CONFIG_DONGLE_SCREEN_BATTERY_WARN_LEVEL) {
    ...
    lv_canvas_set_px(canvas, 0, 0, lv_color_black(), LV_OPA_100);
    lv_canvas_set_px(canvas, 0, 4, lv_color_black(), LV_OPA_100);
    lv_canvas_set_px(canvas, 101, 0, lv_color_black(), LV_OPA_100);
...
static void set_battery_symbol(lv_obj_t* widget, struct battery_state state) {
    ...
    if (state.level < CONFIG_DONGLE_SCREEN_BATTERY_CRIT_LEVEL) {
        lv_obj_set_style_text_color(label, lv_palette_main(LV_PALETTE_RED), 0);
        lv_label_set_text(label, "X");
    } else if (state.level <= CONFIG_DONGLE_SCREEN_BATTERY_WARN_LEVEL) {
